...
```{r}
# Hi Jim, somehow I made it but used the very entrance level code and it is too long.. I'm worried if our points will be cut due to this. Have any idea to simplify it? Anything you recall we can use from the class this term?
...
head(d)
#code still on Jim's local - trying to push again back to Claire 
```
...
```{r}
# Hi Jim, I don't think we should keep this chunck?

head(d)

d_students <- d %>% 
  filter(hispanic_latino >= 1 &
        white >= 1)

head(d_students)

d_students[1:1904, ] %>% 
  estimate_v("white", "hispanic_latino")

#I'm not sure if this worked: It appeared to run correctly, but a value never populated. 

districts <- unique(v$district)[1:1904]
...
```{r}
# Hi Jim, I think we should keep this chunck. I understand it gives an error message but the d_year_district should be what we want.

# the major problem in part B question 1 is that we need to run the estimate_v function within each district for each year; I tried a lot but still couldn't figure out how to use split() twice to split by district then by year...FORTUNATELY, we learned this from week 5 class. we can use nest(). Pretty neat!

# based on the example, estimate_v(d[1:4, ], "white", "hispanic_latino"), we map through the full dataset:
...
# extract the "v"
d_year_distr$v <- map(d_year_distr$gap, 2)

v <- d_year_distr %>% 
  select(academic_year, district, gap = v)
...
```{r}
# Hi Jim, I tried to do one plot but didn't get close....geom_line and geom_path won't work. don't know how to reproduce the plot in the assignment!

plot1 <- for_plot %>% 
  filter(district == "Bandon SD 54") %>%
  mutate(year = as.character(academic_year), g = as.character(gap)) %>% 
  ggplot(aes(year, g))+
...
```{r school district plots}

d_year_distr <- d %>%
  group_by(academic_year, district) %>% 
	nest() %>%
	mutate(gap = map(data, ~estimate_v(.x, "white", "hispanic_latino"))) %>% 
  ggplot(.x, aes(academic_year, gap)) +
  geom_errorbar(aes(ymin = gap + qnorm(0.025),
                    ymax = gap + qnorm(0.975)),
                color = "gray40") +
  geom_point()
